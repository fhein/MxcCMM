...
namespace MxcCommons\Loader;

use Traversable;

if (class_exists('MxcCommons\Loader\AutoloaderFactory')) {
    return;
}

abstract class AutoloaderFactory
{
    const STANDARD_AUTOLOADER = 'MxcCommons\Loader\StandardAutoloader';
     ...
     * loaded).
     *
     * @param  array|Traversable $options (optional) options to use. Defaults to MxcCommons\Loader\StandardAutoloader
     * @return void
    ...
    public static function factory($options = null)
    {
        ...
        foreach ($options as $class => $autoloaderOptions) {
            if (! isset(static::$loaders[$class])) {
                ...
                if (! is_subclass_of($class, 'MxcCommons\Loader\SplAutoloader')) {
                    require_once 'Exception/InvalidArgumentException.php';
                    throw new Exception\InvalidArgumentException(
                        sprintf('Autoloader class %s must implement MxcCommons\\Loader\\SplAutoloader', $class)
                    );
                }
