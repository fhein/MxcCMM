...
/**
 * An adapter class for MxcCommons\Http\Client based on the curl extension.
...
class Curl implements HttpAdapter, StreamInterface
{
    ...
    public function __construct()
    {
        if (! extension_loaded('curl')) {
            throw new AdapterException\InitializationException(
                'cURL extension has to be loaded to use this MxcCommons\Http\Client adapter'
            );
        }
    ...
    public function connect($host, $port = 80, $secure = false)
    {
        ...
        if ($secure !== false) {
            // Behave the same like MxcCommons\Http\Adapter\Socket on SSL options.
    ...
    public function write($method, $uri, $httpVersion = 1.1, $headers = [], $body = '')
    {
        ...
        // disallow the MxcCommons\Http\Response to do it again.
