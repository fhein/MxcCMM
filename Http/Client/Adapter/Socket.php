...
namespace MxcCommons\Http\Client\Adapter;

use MxcCommons\Http\Client\Adapter\AdapterInterface as HttpAdapter;
use MxcCommons\Http\Client\Adapter\Exception as AdapterException;
use MxcCommons\Http\Request;
use MxcCommons\Http\Response;
use MxcCommons\Stdlib\ArrayUtils;
use MxcCommons\Stdlib\ErrorHandler;
use MxcCommons\Uri\Uri;
use Traversable;

/**
 * A sockets based (stream\socket\client) adapter class for MxcCommons\Http\Client. Can be used
...
class Socket implements HttpAdapter, StreamInterface
{
    ...
    public function setOptions($options = [])
    {
        ...
        if (! is_array($options)) {
            throw new AdapterException\InvalidArgumentException(
                'Array or MxcCommons\Config object expected, got ' . gettype($options)
            );
        }
    ...
    public function read()
    {
        ...
        if ($transferEncoding !== false) {
            ...
            // this means we have to disallow the MxcCommons\Http\Response to do it again
            if ($this->outStream) {
