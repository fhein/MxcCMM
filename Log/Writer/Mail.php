...
use MxcCommons\Log\Formatter\Simple as SimpleFormatter;
use MxcCommons\Mail\Message as MailMessage;
use MxcCommons\Mail\MessageFactory as MailMessageFactory;
use MxcCommons\Mail\Transport;
use MxcCommons\Mail\Transport\Exception as TransportException;
use Traversable;

/**
 * Class used for writing log messages to email via MxcCommons\Mail.
 *
 * Allows for emailing log messages at and above a certain level via a
 * MxcCommons\Mail\Message object.  Note that this class only sends the email upon
 * completion, so any log entries accumulated are sent in a single email.
 * The email is sent using a MxcCommons\Mail\Transport\TransportInterface object
 * (Sendmail is default).
 */
class Mail extends AbstractWriter
{
     ...
     *
     * Can only be used of the MxcCommons\Mail object has not already had its
    ...
    public function __construct($mail, Transport\TransportInterface $transport = null)
    {
        ...
        if (! $mail instanceof MailMessage) {
            throw new Exception\InvalidArgumentException(sprintf(
                'Mail parameter of type %s is invalid; must be of type MxcCommons\Mail\Message',
        ...
        if (! $transport instanceof Transport\TransportInterface) {
            throw new Exception\InvalidArgumentException(sprintf(
                'Transport parameter of type %s is invalid; must be of type MxcCommons\Mail\Transport\TransportInterface',
     ...
     * level appended to the end.  Since a MxcCommons\Mail\Message subject can only be set
     * once, this method cannot be used if the MxcCommons\Mail\Message object already has a
     * subject set.
     *
    ...
    public function shutdown()
    {
        ...
        if ($this->subjectPrependText !== null) {
            ...
            // line and set it on the MxcCommons\Mail object.
